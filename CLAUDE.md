...
- Tipos usados: `new_training`, `training_replay`, `training_live_expired`, `announcement`, `new_feedback`, `new_member_pending`
- **Realtime + polling (híbrido)**: `notifications` está na publication `supabase_realtime` (migração `033`) — usado por `src/components/admin/admin-notification-sound.tsx` para tocar som (`public/sounds/new-ticket.mp3`) + toast quando chega notificação `type: 'new_feedback'` ou `'new_member_pending'`. Componente é montado UMA VEZ em `src/app/(admin)/admin/layout.tsx` (não dentro da sidebar, que duplica para mobile/desktop — evita assinatura/som duplicados). **Realtime sozinho não é confiável** (confirmado empiricamente: entrega intermitente do evento `postgres_changes`, mesmo com a tabela corretamente na publication e RLS ok) — por isso o componente também faz polling a cada 15s via `getRecentSoundNotifications` (`src/app/actions/notifications.ts`) como garantia, deduplicando por `id` num `Set` em ref para não tocar duas vezes se ambas as vias pegarem o mesmo evento. Se precisar de realtime em outra tabela, mesma receita de publication: `ALTER PUBLICATION supabase_realtime ADD TABLE x` (idempotente com `DO $$ ... EXCEPTION WHEN duplicate_object`) — mas considere se vale a pena também ter um fallback de polling.
